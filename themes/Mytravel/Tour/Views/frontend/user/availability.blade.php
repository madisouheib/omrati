...
@section ('content')
            ...
            <div class="filter-div d-flex justify-content-between ">
                <div class="col-left">
                    <form method="get" action="" class="filter-form filter-form-left d-flex flex-column flex-sm-row" role="search">
                        <input type="text" name="s" value="{{ Request()->s }}" placeholder="{{__('البحث بالاسم')}}" class="form-control">&nbsp;&nbsp;
                        <button class="btn-info btn btn-icon btn_search btn-sm" type="submit">{{__('بحث')}}</button>
                    </form>
                </div>
                <div class="col-right">
                    @if($rows->total() > 0)
                        <span class="count-string">{{ __("عرض :from - :to من :total جولات",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="user-panel">
        <div class="panel-title"><strong>{{__('الإتاحة')}}</strong></div>
    ...
    @else
        <div class="alert alert-warning">{{__("لم يتم العثور على جولات")}}</div>
    @endif
        ...
        <div class="modal-dialog modal-lg  modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{__('معلومات التاريخ')}}</h5>
                    ...
                    <form class="row form_modal_calendar form-horizontal" novalidate onsubmit="return false">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <br>
                                <label ><input true-value=1 false-value=0 type="checkbox" v-model="form.active"> {{__('متاحة للحجز؟')}}</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label >{{__('أقصى عدد من الضيوف')}}</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" @click="saveForm">{{__('حفظ التغييرات')}}</button>
                </div>
            </div>
        </div>
    </div>
