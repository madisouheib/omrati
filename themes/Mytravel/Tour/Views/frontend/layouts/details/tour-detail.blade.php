<div class="d-block d-md-flex flex-center-between align-items-start mb-3">
    <div class="mb-1">
        <div class="mb-2 mb-md-0">
            <h1 class="font-size-26 font-weight-bold mb-1 mr-3">{!! clean($translation->title) !!}</h1>
        </div>
                ...
                <div class="ml-2 service-review review-{{$score_total}}">
                    <div class="d-inline-flex align-items-center font-size-17 text-lh-1 text-primary">
                        ...
                        @if($reviewData['total_review'] > 1)
                                {{ __("تقييمات :number",["number"=>$reviewData['total_review'] ]) }}
                            @else
                                {{ __("تقييم :number",["number"=>$reviewData['total_review'] ]) }}
                            @endif
                    </span>
                    </div>
                </div>
    ...
    <ul class="list-group list-group-borderless list-group-horizontal row">
        ...
        @if($row->duration)
            <li class="col-md-4 flex-horizontal-center list-group-item text-lh-sm mb-2 border-0">
                ...
                <div class="ml-1 text-gray-1">{{ __("الحد الأقصى للأشخاص") }} : {{ $row->max_people }}</div>
            </li>
        @endif
        @if($row->wifi_available)
            <li class="col-md-4 flex-horizontal-center list-group-item text-lh-sm mb-2 border-0">
                <i class="flaticon-wifi-signal text-primary font-size-22 mr-2 d-block"></i>
                <div class="ml-1 text-gray-1">{{ __("واي فاي متوفر") }}</div>
            </li>
        @endif
        @if($row->date_form_to)
            <li class="col-md-4 flex-horizontal-center list-group-item text-lh-sm mb-2 border-0">
        ...
        @if($row->min_age)
            <li class="col-md-4 flex-horizontal-center list-group-item text-lh-sm mb-2 border-0">
                ...
                <div class="ml-1 text-gray-1 ">{{ __('الحد الأدنى للعمر:') }} {{ $row->min_age }}</div>
            </li>
        @endif
        @if($row->pickup)
            <li class="col-md-4 flex-horizontal-center list-group-item text-lh-sm mb-2 border-0">
                <i class="flaticon-pin text-primary font-size-22 mr-2 d-block"></i>
                <div class="ml-1 text-gray-1">{{ __("الانطلاق من:") }} {{ __("$row->pickup") }}</div>
            </li>
        @endif
    </ul>
</div>
