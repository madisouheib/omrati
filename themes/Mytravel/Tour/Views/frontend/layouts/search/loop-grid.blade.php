...
<div class="card transition-3d-hover shadow-hover-2 item-loop h-100 {{$wrap_class ?? ''}}">
    <div class="position-relative mb-2">
        ...
        <div class="position-absolute top-0 left-0 pt-4 pl-3 featured">
            @if($row->is_featured == "1")
                <span class="badge badge-pill bg-white text-primary px-4 mr-3 py-2 font-size-14 font-weight-normal">{{ __("مميز") }}</span>
            @endif
        ...
        <div class="position-absolute top-0 right-0 pt-4 pr-3 btn-wishlist">
            <button type="button" class="p-0 btn btn-sm btn-icon text-white rounded-circle service-wishlist {{$row->isWishList()}}" data-id="{{$row->id}}" data-type="{{$row->type}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="{{ __("حفظ لوقت لاحق") }}">
        ...
        <div class="position-absolute bottom-0 left-0 right-0 text-content">
            <div class="px-3 pb-2">
                ...
                <h2 class="h5 text-white mb-0 font-weight-bold">
                    <small class="mr-1 font-size-14">{{ __("ابتداءا من") }}</small>
    ...
    <div class="card-body px-3 py-2">
                ...
                <div class="d-inline-flex align-items-center font-size-17 text-lh-1 text-primary">
                    ...
                    <span class="text-secondary font-size-14 mt-1">
                        @if($reviewData['total_review'] > 1)
                            {{ __(":number تقييمات",["number"=>$reviewData['total_review'] ]) }}
                        @else
                            {{ __(":number تقييم",["number"=>$reviewData['total_review'] ]) }}
                        @endif
                    </span>
                </div>
            </div>
        ...
        <div class="g-price d-none">
            <div class="prefix">
                <span class="fr_text">{{__(" ابتداءا من")}}</span>
            </div>
            <div class="price">
