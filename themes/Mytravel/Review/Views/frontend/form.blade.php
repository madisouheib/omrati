                    ...
                    <div class="col-md-4 mb-4 mb-md-0">
                        <div class="border rounded flex-content-center py-5 border-width-2">
                            <div class="text-center">
                                ...
                                <div class="text-gray-1">{{__("من")}}
                                    @if($review_score['total_review'] > 1)
                                        {{ __(":number تقييمات",["number"=>$review_score['total_review'] ]) }}
                                    @else
                                        {{ __(":number تقييم",["number"=>$review_score['total_review'] ]) }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="border-bottom py-4">
            @if($review_list->total() > 0)
                <h5 class="font-size-21 font-weight-bold text-dark mb-5 mt-3">
                    {{ __("عرض :from - :to من :total الإجمالي",["from"=>$review_list->firstItem(),"to"=>$review_list->lastItem(),"total"=>$review_list->total()]) }}
                </h5>
            @else
        ...
        <div class="py-4">
                        ...
                        <div class="row mb-5 mb-lg-0">
                            ...
                            <div class="col-sm-12 mb-5">
                                <div class="js-form-message">
                                    <input type="text" class="form-control" name="review_title" placeholder="{{__("العنوان")}}" required data-error-class="u-has-error" data-msg="{{__('العنوان مطلوب')}}" data-success-class="u-has-success">
                                    <div class="invalid-feedback">{{__('العنوان مطلوب')}}</div>
                                </div>
                            </div>
                            <div class="col-sm-12 mb-5">
                                <div class="js-form-message">
                                    <div class="input-group">
                                        <textarea class="form-control" rows="6" cols="77" name="review_content" placeholder="{{__("محتوى التقييم")}}" required data-msg="{{__('محتوى التقييم يجب أن يحتوي على 10 أحرف على الأقل')}}" data-error-class="u-has-error" data-success-class="u-has-success"></textarea>
                                        <div class="invalid-feedback">{{__('محتوى التقييم يجب أن يحتوي على 10 أحرف على الأقل')}}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col d-flex justify-content-center justify-content-lg-start">
                                <button type="submit" id="submit" name="submit" class="btn rounded-xs bg-blue-dark-1 text-white p-2 height-51 width-190 transition-3d-hover">{{__("ترك تقييم")}}</button>
            ...
            @if(!Auth::id())
                <div class="review-message">
                    {!!  __("يجب أن تكون <a href='#login' data-toggle='modal' data-target='#login'>مسجل الدخول</a> لكتابة تقييم") !!}
                </div>
            @endif
        </div>
    </div>
