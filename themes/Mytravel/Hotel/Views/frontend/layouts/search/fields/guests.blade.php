    ...
    <div class="col dropdown-custom px-0 mb-4 form-select-guests">
        ...
        <div class="flex-horizontal-center border-bottom border-width-2 border-color-1 py-2 d-flex dropdown-toggle" data-toggle="dropdown">
            ...
            <div class="text-black font-size-16 font-weight-semi-bold mr-auto">
                <div class="render">
                    <span class="adults"><span class="one @if($adults >1) d-none @endif">{{__('بالغ 1')}}</span> <span class="@if($adults <= 1) d-none @endif multi" data-html="{{__(':count بالغين',['count'=>request()->query('adults',1)])}}">{{__(':count بالغين',['count'=>request()->query('adults',1)])}}</span></span>
                    -
                    <span class="children">
                        <span class="one @if($children >1) d-none @endif" data-html="{{__(':count طفل',['count'=>request()->query('children',0)])}}">{{__(':count طفل',['count'=>request()->query('children',0)])}}</span>
                        <span class="multi @if($children <=1) d-none @endif" data-html="{{__(':count أطفال',['count'=>request()->query('children',0)])}}">{{__(':count أطفال',['count'=>request()->query('children',0)])}}</span>
                    </span>
                </div>
            </div>
        </div>
        <div class="dropdown-menu select-guests-dropdown">
            <div class="dropdown-item-row">
                <div class="label">{{__('البالغين')}}</div>
                <div class="val">
            ...
            <div class="dropdown-item-row">
                <div class="label">{{__('الأطفال')}}</div>
                <div class="val">
