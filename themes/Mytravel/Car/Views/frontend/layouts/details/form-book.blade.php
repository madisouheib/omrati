            ...
            <div class="border border-color-7 rounded mb-5">
                <div class="border-bottom">
                    @if($row->discount_percent)
                        <div class="sale-box">
                            <div class="ribbon ribbon--red">{{ __("وفر :text",['text'=>$row->discount_percent]) }}</div>
                        </div>
                    @endif
                    <div class="p-4">
                        <span class="font-size-14">{{ __("ابتداءا من") }}</span>
                ...
                <div class="nav-enquiry" v-if="is_form_enquiry_and_book">
                    ...
                    <div class="enquiry-item" data-toggle="modal" data-target="#enquiry_form_modal">
                        <span>{{ __("استفسار") }}</span>
                    </div>
                </div>
                <div class="form-book" :class="{'d-none':enquiry_type!='book'}">
                    <div class="p-4">
                        <span class="d-block text-gray-1 font-weight-normal mb-0 text-left">{{ __("تاريخ الاستلام") }}</span>
                        <div class="mb-4">
                                    ...
                                    <div class="render check-in-render mb-1">
                                        <small>
                                            @if($row->min_day_before_booking > 1)
                                                - {{ __("احجز :number أيام مقدمًا",["number"=>$row->min_day_before_booking]) }}
                                            @else
                                                - {{ __("احجز :number يوم مقدمًا",["number"=>$row->min_day_before_booking]) }}
                                            @endif
                                        </small>
                                    </div>
                                @endif
                                    ...
                                    <div class="render check-in-render mb-1">
                                        <small>
                                            @if($row->min_day_stays > 1)
                                                - {{ __("البقاء لمدة :number أيام على الأقل",["number"=>$row->min_day_stays]) }}
                                            @else
                                                - {{ __("البقاء لمدة :number يوم على الأقل",["number"=>$row->min_day_stays]) }}
                                            @endif
                                        </small>
                                    </div>
                                @endif
                        ...
                        <div class="mb-4 border-bottom border-width-2 border-color-1 pb-1" v-if="extra_price.length">
                            <h4 class="flex-center-between mb-1 font-size-16 text-dark font-weight-bold">{{__('أسعار إضافية:')}}</h4>
                        ...
                        <ul class="form-section-total mb-4  list-unstyled  pb-1" v-if="total_price > 0">
                            <li>
                                <label>{{__("المجموع")}}</label>
                                <span class="price">@{{total_price_html}}</span>
                            </li>
                            <li v-if="is_deposit_ready">
                                <label for="">{{__("ادفع الآن")}}</label>
                        ...
                        <div class="text-center">
                            <button class="btn btn-primary d-flex align-items-center justify-content-center  height-60 w-100 mb-xl-0 mb-lg-1 transition-3d-hover font-weight-bold" @click="doSubmit($event)" :class="{'disabled':onSubmit,'btn-success':(step == 2),'btn-primary':step == 1}" name="submit">
                                <span class="stop-color-white">{{__("احجز الآن")}}</span>
