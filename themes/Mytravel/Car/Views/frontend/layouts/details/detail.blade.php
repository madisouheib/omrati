    ...
    <div class="mb-3">
        <div class="d-block d-md-flex flex-horizontal-center mb-2 mb-md-0">
            ...
            @if($row->getReviewEnable())
                ...
                <span class="font-size-14 text-gray-1 ml-1">{{__(":number تقييم",['number'=>$review_score['total_review']])}}</span>
            @endif
    ...
    <ul class="list-group list-group-horizontal custom-social-share">
        ...
        <li class="list-group-item px-1 border-0">
            ...
            <div id="shareDropdown{{$row->id}}" class="dropdown-menu dropdown-unfold dropdown-menu-right mt-0 px-3 min-width-3" aria-labelledby="shareDropdownInvoker{{$row->id}}">
                <a class="btn btn-icon btn-pill btn-bg-transparent transition-3d-hover  btn-xs btn-soft-dark  facebook mb-3" href="https://www.facebook.com/sharer/sharer.php?u={{$row->getDetailUrl()}}&amp;title={{$translation->title}}" target="_blank" rel="noopener" original-title="{{__("فيسبوك")}}">
                    <span class="font-size-15 fa fa-facebook-f btn-icon__inner"></span>
                </a>
                <br/>
                <a class="btn btn-icon btn-pill btn-bg-transparent transition-3d-hover  btn-xs btn-soft-dark  twitter" href="https://twitter.com/share?url={{$row->getDetailUrl()}}&amp;title={{$translation->title}}" target="_blank" rel="noopener" original-title="{{__("تويتر")}}">
    ...
    <ul class="list-group list-group-borderless list-group-horizontal flex-center-between text-center mx-md-4 flex-wrap">
        @if($row->passenger)
            <li class="list-group-item text-lh-sm ">
                ...
                <div class="text-gray-1">{{ $row->passenger }} {{ __("مقاعد") }}</div>
            </li>
        @endif
        @if($row->gear)
            <li class="list-group-item text-lh-sm ">
                <i class="icon-gear text-primary font-size-50 mb-1 "></i>
                <div class="text-gray-1"> {{$row->gear}} {{ __("علبة التروس") }}</div>
            </li>
        @endif
        @if($row->baggage)
            <li class="list-group-item text-lh-sm ">
                <i class="icon-baggage text-primary font-size-50 mb-1 "></i>
                <div class="text-gray-1"> {{$row->baggage}} {{ __(" حقائب") }}</div>
            </li>
        @endif
        @if(!empty($row->door))
            <li class="list-group-item text-lh-sm ">
                <i class="icon-door text-primary font-size-50 mb-1 "></i>
                <div class="text-gray-1">{{$row->door}} {{ __("أبواب") }}</div>
            </li>
        @endif
    </ul>
</div>
    ...
    <div class="position-relative">
        <div id="sliderSyncingNav" class="travel-slick-carousel u-slick mb-2"
            ...
            @foreach($row->getGallery() as $key=>$item)
                <div class="js-slide">
                    <img class="img-fluid border-radius-3" src="{{$item['large']}}" alt="{{ __("معرض الصور") }}">
                </div>
            @endforeach
        </div>
        @if(!empty($row->video))
                ...
                <div class="flex-horizontal-center">
                    <a class="travel-fancybox btn btn-white transition-3d-hover py-2 px-md-5 px-3 shadow-6 mr-1" href="javascript:;"
                       ...
                       data-speed="700">
                        ...
                        <span class="d-none d-md-inline">{{ __("فيديو") }}</span>
                    </a>
                </div>
            </div>
        @endif
        <div id="sliderSyncingThumb" class="travel-slick-carousel u-slick u-slick--gutters-4 u-slick--transform-off"
            ...
            @foreach($row->getGallery() as $key=>$item)
                <div class="js-slide" style="cursor: pointer;">
                    <img class="img-fluid border-radius-3 height-110" src="{{$item['thumb']}}" alt="{{ __("معرض الصور") }}">
                </div>
            @endforeach
        </div>
    </div>
...
@if($translation->faqs)
    <div class="g-faq">
        <h3 class="font-size-21 font-weight-bold text-dark mb-4"> {{__("الأسئلة الشائعة")}} </h3>
...
@if($row->map_lat && $row->map_lng)
    <div class="mt-4">
        <h3 class="font-size-21 font-weight-bold text-dark mb-4">{{__("الموقع")}}</h3>
